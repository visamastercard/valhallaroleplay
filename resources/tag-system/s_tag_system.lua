	...
	if (exports.global:isPlayerAdmin(thePlayer)) then
		...
		for key, value in ipairs(exports.global:getNearbyElements(thePlayer, "object")) do
			...
			if (objtype=="tag") then
				...
				local creator = getElementData(value, "creator")
				local creatorName = "Unknown"
				if (creatorName>0) then
					ownerName = exports['cache']:getCharacterName(creatorName, true)
				end
				outputChatBox("  ID "..id ..". Created by " .. ownerName, thePlayer, 255, 126, 0)
				count = count + 1
			end
		end
		if (count==0) then
...
function setTag(thePlayer, commandName, newTag)
	...
	elseif getElementData(thePlayer, "tag") == 9 then
		...
		if (newTag>0) and (newTag<9) then
			...
			if (teamName~="Southside Crips") and (newTag==3) then -- Crips
