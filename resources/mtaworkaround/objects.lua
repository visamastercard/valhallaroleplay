...
local function checkStreamIn()
	if ( enabled ) then
		...
		for key, value in pairs(getElementsByType(element, getResourceDynamicElementRoot(getResourceFromName ( "item-system")))) do
			local elementDimension = getElementDimension(value)

			if elementDimension == playerdimension then
				...
				if (distx < streamdistance) or (disty < streamdistance) then
