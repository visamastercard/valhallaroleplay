...
addEventHandler( "onClientResourceStart", getResourceRootElement( ),
	function( )
		if getElementData( getLocalPlayer( ), "faction" ) == 1 and ( getElementData( getLocalPlayer( ) ) or 0 ) > 0 then
			for key, value in pairs( getElementsByType( "player" ) ) do
				if value ~= getLocalPlayer( ) and not blips[ value ] and getElementData( value, "faction" ) == 1 and ( getElementData( value ) or 0 ) > 0 then
	...
	function( name )
		if ( name == "faction" or name == "duty" ) and getElementType( source ) == "player" then
			...
			if source == getLocalPlayer( ) then
				if newfaction ~= 1 or newduty == 0 then
			...
			elseif getElementData( getLocalPlayer( ), "faction" ) == 1 then
				if newfaction == 1 and newduty > 0 then
