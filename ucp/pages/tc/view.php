										...
										if(!$gi)
										{
											...
											if($userID == $gi['creator'])
											{
												...
												<tr>
													...
													<td width="10%" align="left" valign="top"><b>Assiged:</b></td>
											...
											if($gi['status'] == 3)
											{
												echo('<p>To reopen your ticked, just reply using the form below.</p>');
											}
									...
									if(isset($_POST['Do']))
									{
										...
										if(isset($_POST['assign_admin']) && $_POST['assign_admin'] != $c['assigned'])
										{
											...
											if($su)
											{
												...
												$smtp->send($gpi['email'], 'vG MTA Ticket Center - Ticket #'.$_GET["view"].' Assigned', 'Hello ' . $gpi['username'] . ',
...
The ValhallaGaming MTA Administration Team');
												...
												$smtp->send($gpi2['email'], 'vG MTA Ticket Center - Ticket #'.$_GET["view"].' Assigned', 'Hello ' . $gpi2['username'] . ',
										...
										if(isset($_POST['admin_action']))
										{
											if($_POST['admin_action'] == 1)
											{
												...
												if($su)
												{
													...
													$smtp->send($gpi['email'], 'Ticket #'.$_GET["view"].' Closed - vG MTA Ticket Center', 'Hello ' . $gpi['username'] . ',
...
The ValhallaGaming MTA Administration Team');
											...
											while($sin = mysql_fetch_array($in))
											{
												echo('<div style="border:1px solid #FF6600;">
														...
														<div style="text-align:left;padding:2px;">' . nl2br($sin["message"]) . '</div>
													</div>');
											}
